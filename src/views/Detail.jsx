...
export default function Detail() {
  ...
  const productDetailQuantity = cartItems.filter(
  ...
  const relatedItems = products.filter(
    (product) => product.category === productDetail[0].category
  );
      ...
      <ContainerDetail>
        ...
        <ContainerTextAndNumbers>
          ...
          <ContainerPriceAndAdd>
            ...
            <AddOrSub>
              <BtnMore onClick={() => onRemove(productDetail[0])}>
              ...
              <BtnMore onClick={() => onAdd(productDetail[0])}>
          ...
          <DescriptionContainer>
            ...
            <DescriptionTex>
              ...
              Voluptates atque itaque voluptatem magnam.
            </DescriptionTex>
          </DescriptionContainer>
        </ContainerTextAndNumbers>
          ...
          <RelatedItems>
            {relatedItems.map((item) => (
              <Items key={item.id}>
                <ImageItem
                  src={item.image}
                  onClick={() => history(`?productID=${item.id}`)}
                />
                {item.name}
              </Items>
            ))}
        ...
        <ContainerAddCart>
          <AddToCart>
            <CartButton onClick={() => onAdd(productDetail[0])}>
              Add To Cart
            </CartButton>
          </AddToCart>
        </ContainerAddCart>
